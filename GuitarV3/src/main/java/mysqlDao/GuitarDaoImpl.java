...
public class GuitarDaoImpl implements GuitarDao {
	...
	public List<Guitar> search(GuitarSpec spec) {
        ...
        ResultSet rs=DbutilMysql.executeQuery("select * from guitar",new Object[]{});
        try {
            while(rs.next()){
	...
	public boolean add(Guitar guitar) {
		...
		return DbutilMysql.executeUpdate(sql, new Object[]{guitar.getSerialNumber(),guitar.getPrice(),guitar.getSpec().getBuilder(),guitar.getSpec().getModel(),guitar.getSpec().getType(),guitar.getSpec().getNumStrings(),guitar.getSpec().getBackWood(),guitar.getSpec().getTopWood()});		
	}
	@Override
	public boolean delete(Guitar guitar) {
		// TODO Auto-generated method stub
		String sql = "delete from guitar where serialNumber=?";
return DbutilMysql.executeUpdate(sql, new Object[]{guitar.getSerialNumber()});		
	}
